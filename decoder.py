...
class FeatureDecoder(nn.Module):
    def __init__(self, feat_map_nums: int, width=768, sbn=True):   # todo: the decoder's width follows a simple halfing rule; you can change it to any other rule
        super().__init__()
        self.width = width
        channels = [self.width // 2 ** i for i in range(feat_map_nums)] # todo: the decoder's width follows a simple halfing rule; you can change it to any other rule
        bn2d = nn.SyncBatchNorm if sbn else nn.BatchNorm2d
        self.dec = nn.ModuleList([UNetBlock(cin, cout, bn2d) for (cin, cout) in zip(channels[:-1], channels[1:])])
    ...
    def forward(self, to_dec: List[torch.Tensor]):
        fea_dec = [self.gap(to_dec[0])]
        x = to_dec[0]
        for i, d in enumerate(self.dec):
            x = self.dec[i](x)
            x = x + to_dec[i+1]
            fea_dec.append(self.gap(x))
        return fea_dec
