...
def main_pt():
    ...
    dataset_train = build_dataset_to_pretrain(args.data_path, args.input_size, args.hyp)
    ...
    model_without_ddp = FCVR(
        sparse_encoder=sparse_enc, dense_decoder=pixel_dec, momentum_encoder=momentum_enc, feature_decoder=feature_dec, mask_ratio=args.mask,
    ...
    if ep_start >= args.ep: # load from a complete checkpoint file
        ...
        for ep in range(ep_start, args.ep):
            ...
            misc.save_checkpoint_with_meta_info_and_opt_state(f'{args.model}_withdecoder_1kpretrained_fcvr_style.pth', args, ep, performance_desc, model_without_ddp.state_dict(with_config=True), optimizer.state_dict())
...
def pre_train_one_ep(ep, args: arg_util.Args, tb_lg: misc.TensorboardLogger, itrt_train, iters_train, model: DistributedDataParallel, optimizer):
    ...
    for it, (inp1, inp2) in enumerate(me.log_every(iters_train, itrt_train, 3, header)):
        # adjust lr and wd
        ...
        inp1 = inp1.to(args.device, non_blocking=True)
        inp2 = inp2.to(args.device, non_blocking=True)
        FCVR.forward
        loss = model(inp1, inp2, active_b1ff=None, vis=False)[0]
        optimizer.zero_grad()
        loss.backward()

        with torch.no_grad():
            for p1, p2 in zip(model.module.sparse_encoder.parameters(), model.module.momentum_encoder.parameters()):
                p2.data = 0.997 * p2.data + 0.003 * p1.data
