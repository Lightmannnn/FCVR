...
class MomentumEncoder(nn.Module):
    def __init__(self, cnn, decoder, sbn=False, verbose=False):
        super().__init__()
        self.cnn = cnn
        self.decoder = decoder
        self.downsample_ratio, self.e_widths = cnn.get_downsample_ratio(), cnn.get_feature_map_channels()
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.contra_projs = nn.ModuleList()
        d_width = self.decoder.width
        for i in range(len(self.e_widths)):
            e_width = self.e_widths.pop()
            if i <= 0 and e_width == d_width:
                contra_proj = nn.Identity()
            else:
                kernel_size = 1 if i <= 0 else 3
                contra_proj = nn.Conv2d(e_width, d_width, kernel_size=kernel_size, stride=1, padding=kernel_size // 2, bias=True)
            self.contra_projs.append(contra_proj)
            d_width //= 2


    def forward(self, x):
        res = self.cnn(x, hierarchical=True)
        res.reverse()
        output = []
        for i in range(len(res)):
            output.append(self.gap(self.contra_projs[i](res[i]))) # gap after projection
        return output